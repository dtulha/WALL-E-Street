...
def run_hedge_fund(
    ...
    try:
        # Always use our four main analysts
        default_analysts = ['warren_buffett', 'ben_graham', 'cathie_wood', 'bill_ackman']
        ...
        # Get all analyst signals and portfolio manager decisions
        analyst_signals = final_state["data"]["analyst_signals"]
        portfolio_decisions = parse_hedge_fund_response(final_state["messages"][-1].content)

        # Structure the response by analyst
        responses = {}
        
        # Add individual analyst responses
        for analyst in default_analysts:
            analyst_name = analyst.replace('_', ' ').title()
            analyst_data = analyst_signals.get(analyst + "_agent", {})
            
            responses[analyst_name] = {
                "signals": {
                    ticker: {
